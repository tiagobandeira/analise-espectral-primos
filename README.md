...
As Notas 16–26 constituem um corpo completo e autocontido:
...
A natureza do critério $\rho_B$ está completamente esclarecida (Nota 26): é um critério geométrico no espaço logarítmico, implementável sem aritmética inteira via $\rho_{\text{adapt}}$, e a aritmética inteira é sua implementação ótima — não um ingrediente irredutível.
...
4. **Cota inferior de $\rho_{\text{adapt}}$ para primos** — o valor mínimo observado é $\rho_{\min} \approx 1{,}4 \times 10^{-4}$ para $k = 9$. A conexão com cotas efetivas de Baker para formas lineares em logaritmos forneceria garantia analítica (em vez de empírica) para a robustez de $\rho^* = 10^{-6}$ (Nota 26, Questão 1).

5. **Ressonância espectral como detector de fatores** — pode-se detectar diretamente a existência de um fator primo comum entre dois números pela interferência de seus picos, evitando qualquer divisibilidade? (Hipótese B, Nota 20.)
